...
if( Random(0,100) < 66 ) then
	...
	if( e_id ~= 0 ) then
		...
		if( distance_sqrt < spawn_distance * spawn_distance ) then
			...
			if (spawned_entities < 7) then
				-- spawn enemy
				pos_x = pos_x + Random(-4,4)
				pos_y = pos_y + Random(-4,4)
				
				local thisBee = "data/entities/animals/nv_bee.xml"
				
				if (Random(0,100) < 50) then
					thisBee = "data/entities/animals/nv_bee_stinger.xml"
				end

				local new_entity = EntityLoad( thisBee, pos_x - 3, pos_y - 4 )
